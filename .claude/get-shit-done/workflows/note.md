...
- **Global scope**: `C:/Shiv/Projects/AIML/FundLens/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/Shiv/Projects/AIML/FundLens/.claude/notes/`)
...
2. Glob `C:/Shiv/Projects/AIML/FundLens/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/Shiv/Projects/AIML/FundLens/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/Shiv/Projects/AIML/FundLens/.claude/notes/` — works in any directory
